...
describe('I-REC Registration tests', () => {
    ...
    it('should create and return new IREC connection', async () => {
        ...
        accounts.forEach((account) => {
            ...
            expect(account.details.countryCode).to.be.a('string');
            expect(account.details.private).to.be('boolean');
            expect(account.details.restricted).to.be('boolean');
            expect(account.details.active).to.be('boolean');
        });
    });
});
