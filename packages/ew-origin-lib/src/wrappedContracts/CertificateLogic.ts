...
export class CertificateLogic extends CertificateSpecificContract {
    ...
    async splitAndPublishForSale(_certificateId: number, _power: number, _price: number, _tokenAddress: string, txParams?: SpecialTx) {
        let transactionParams;

        const txData = await this.web3Contract.methods
            .splitAndPublishForSale(_certificateId, _power, _price, _tokenAddress)
            .encodeABI();
