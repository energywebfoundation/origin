...
describe('MarketContractLookup API tests', async () => {
    ...
    describe('GET', () => {
        it('gets empty array when no contract addresses were set', async () => {
            const getResult: AxiosResponse = await axios.get(
                `${BASE_API_URL}/MarketContractLookup`
            );
        ...
        it('returns all contract addresses', async () => {
            await axios.post(`${BASE_API_URL}/MarketContractLookup`, {
                value: marketContractLookup
            });
            await axios.post(`${BASE_API_URL}/MarketContractLookup`, {
                value: marketContractLookup2
            });
            ...
            assert.deepEqual(getResult.data, [marketContractLookup, marketContractLookup2]);
        });
    });

    describe('POST', () => {
        it('creates a MarketContractLookup', async () => {
            const postResult = await axios.post(`${BASE_API_URL}/MarketContractLookup`, {
                value: marketContractLookup
            });
            ...
            assert.equal(
                ...
                `MarketContractLookup ${marketContractLookup} created`
            );
        });
    ...
    describe('DELETE', () => {
        it('deletes a marketContractLookup', async () => {
            ...
            const deleteResult = await axios.delete(`${BASE_API_URL}/MarketContractLookup`, {
                data: { value: marketContractLookup }
            });
            ...
            try {
                await axios.delete(`${BASE_API_URL}/MarketContractLookup`, {
                    data: { value: marketContractLookup }
                });
            } catch (error) {
