import { IDevice, DeviceUpdateData, ISmartMeterRead, IDeviceWithId } from '@energyweb/origin-backend-core';
import { IRequestClient, RequestClient } from './RequestClient';

export interface IDeviceClient {
    getById(id: number): Promise<IDevice>;
    getAll(): Promise<IDeviceWithId[]>;
    add(device: IDevice): Promise<IDeviceWithId>;
...
export class DeviceClient implements IDeviceClient {
    ...
    public async getAll(): Promise<IDeviceWithId[]> {
        const { data } = await this.requestClient.get(this.endpoint);

        return data;
    }

    public async add(device: IDevice): Promise<IDeviceWithId> {
