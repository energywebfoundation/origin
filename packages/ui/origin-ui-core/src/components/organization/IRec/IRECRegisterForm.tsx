...
export const IRECRegisterForm = (): JSX.Element => {
  ...
  const numberOfEmployeesOptions = ['1-50', '50-100', '100-300', '300+'].map(
    (option) => {
      return {
        value: option,
        label: option,
  ...
  const onRegister = async (
    ...
    try {
      ...
      const { data: iRecAccount } =
        await iRecClient.organizationClient.register(formData);
  ...
  const VALIDATION_SCHEME = Yup.object({
    ...
    subsidiaries: Yup.string().label(
      t('organization.registration.existingIRECOrg')
    ),
    ...
    leadUserTitle: Yup.string()
      .required()
      .label(t('organization.registration.leadUserTitle')),
    leadUserTitleInput: Yup.string().label(
      t('organization.registration.leadUserTitle')
    ),
                  ...
                  <Grid item container direction="column" xs={12}>
                    ...
                    <Grid item className={classes.inputsHolder}>
                      ...
                      <FormCountrySelect
                        ...
                        currentValue={values.primaryContactOrganizationCountry}
                        onChange={(value) =>
                          setFieldValue(
                            'primaryContactOrganizationCountry',
                            value,
