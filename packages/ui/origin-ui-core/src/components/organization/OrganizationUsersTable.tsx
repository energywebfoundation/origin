...
export function OrganizationUsersTable() {
  ...
  const userIsActive =
    userOffchain && userOffchain.status === UserStatus.Active;
  ...
  async function getPaginatedData({
    ...
    try {
      entities = (
        await organizationClient.getUsers(userOffchain.organization.id)
      ).data;
    } catch (error) {
      const _error = { ...error };
    ...
    newPaginatedData = newPaginatedData.slice(
      offset,
  ...
  const { paginatedData, loadPage, total, pageSize } =
    usePaginatedLoader<IRecord>({
      getPaginatedData,
    });
  ...
  async function changeRole(rowIndex: number) {
    ...
    if (!isRole(userOffchain, Role.OrganizationAdmin)) {
      showNotification(
        `Only the Admin can change user roles.`,
        NotificationTypeEnum.Error
      );
      return;
    }
