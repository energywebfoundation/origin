...
export function CertificateTable(props: IProps): JSX.Element {
  const { currentSort, sortAscending, sortData, toggleSort } =
    usePaginatedLoaderSorting({
      currentSort: {
  ...
  const [selectedCertificates, setSelectedCertificates] = useState<
    ICertificateViewItem[]
  >([]);
  const [detailViewForCertificateId, setDetailViewForCertificateId] =
    useState<number>(null);
  const [showClaimModal, setShowClaimModal] = useState(false);
  const [selectedCertificate, setSelectedCertificate] =
    useState<ICertificateViewItem>(null);
  ...
  async function getPaginatedData({
    ...
    const enrichedData: IEnrichedCertificateData[] = certificates.map(
      (certificate) => {
    ...
    const filteredIEnrichedCertificateData = enrichedData.filter(
      (enrichedCertificateData) => {
        const { source, isOwned, isClaimed } =
          enrichedCertificateData.certificate;
    ...
    const paginatedData = sortedEnrichedData.slice(
      offset,
  ...
  async function claimCertificateBulk(selectedIndexes: string[]) {
    if (selectedIndexes.length === 0) {
      showNotification(
        t('certificate.feedback.zeroSelected'),
    ...
    if (selectedIndexes.length > CERTIFICATE_LIMIT) {
      showNotification(
        t(`certificate.feedback.pleaseSelectLess`, {
          amount: CERTIFICATE_LIMIT,
        }),
        NotificationTypeEnum.Error
      );
  ...
  function getFilters(): ICustomFilterDefinition[] {
    ...
    const filters: ICustomFilterDefinition[] = [
      ...
      {
        property: (record: IEnrichedCertificateData) =>
          record?.producingDevice?.deviceType,
    ...
    [
      {
        ...
        label: t('certificate.properties.deviceType'),
        sortProperties: [
          (record: IEnrichedCertificateData) => record?.deviceTypeLabel,
        ],
      },
      {
        ...
        sortProperties: [
          (record: IEnrichedCertificateData) =>
            record?.producingDevice?.operationalSince,
        ],
      },
  ...
  const rows = paginatedData.map((enrichedData) => {
    ...
    if (enrichedData.producingDevice) {
      deviceType = deviceTypeService?.getDisplayText(
        enrichedData.producingDevice.deviceType
      );
    ...
    const { publicVolume, privateVolume, claimedVolume } =
      enrichedData.certificate.energy;
    ...
    return {
      ...
      certificationDate: formatDate(
        moment.unix(enrichedData.certificate.creationTime)
      ),
  ...
  if (detailViewForCertificateId !== null) {
    return (
      <Redirect
        push={true}
        to={getCertificateDetailsPageUrl(detailViewForCertificateId)}
      />
    );
  }
