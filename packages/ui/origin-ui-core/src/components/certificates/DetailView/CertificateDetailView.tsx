...
export function CertificateDetailView(props: IProps) {
  ...
  const certificationRequestsClient = useSelector(
    getCertificationRequestsClient
  );
  ...
  async function enrichEvent() {
    const { data: allCertificateEvents } =
      await certificatesClient.getAllEvents(selectedCertificate.id);
    const {
    ...
    const jointEvents = allCertificateEvents.map(async (event: any) => {
      ...
      switch (event.name) {
        case 'IssuanceSingle':
          ...
          description = t(
            'certificate.event.description.certificationRequestApproved'
          );
    ...
    const { data: request } =
      await certificationRequestsClient.getByCertificate(
    ...
    setEvents(
      deduplicate(resolvedEvents).sort((a, b) => a.timestamp - b.timestamp)
    );
  }

  const [certificateData, setCertificateData] = useState<
    Array<TCertificateData[]>
  >([]);
  ...
  const fetchDeviceById = async (issuerId: string) => {
    const { data: allDevices }: { data: DeviceDTO[] } =
      await deviceClient.getAll();
        ...
        [
          {
            ...
            data: EnergyFormatter.format(
              selectedCertificate.energy.publicVolume
            ),
          },
          {
        ...
        const claimInfo = [
          {
            ...
            data: EnergyFormatter.format(
              selectedCertificate.energy.claimedVolume
            ) || [''],
          },
        ];
