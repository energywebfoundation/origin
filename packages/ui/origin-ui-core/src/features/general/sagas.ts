...
function createEthereumProviderAccountsChangedEventChannel(
...
function* setupEnvironment(): SagaIterator {
  ...
  try {
    ...
    const environment: IEnvironment = yield call(
      getEnvironmentTask.getEnvironment
    );
...
export function enhanceCertificate(
  ...
  return {
    ...
    source: isCertificateOnExchange
      ? CertificateSource.Exchange
      : CertificateSource.Blockchain,
...
function* findEnhancedExchangeCertificate(
  ...
  let onChainCertificate = onChainCertificates.find(
    (c) => c.tokenId === certificateId
  );
    ...
    if (status === UserStatus.Active) {
      ...
      const enrichedCertificates = allCertificates.map(
        (c): ICertificateViewItem =>
          enhanceCertificate(c, blockchainAccountAddress)
      );
      ...
      for (const certificate of certificates) {
        yield put(
          update ? updateCertificate(certificate) : addCertificate(certificate)
        );
      }
    }
  }
...
function* initializeEnvironment(): SagaIterator {
  while (true) {
    ...
    try {
      ...
      yield put(
        fromUsersActions.setActiveBlockchainAccountAddress(userAddress)
      );
    } catch (error) {
      showNotification(
...
function* checkBlockchainNetwork(): SagaIterator {
  while (true) {
    ...
    if (user && user.status === UserStatus.Active && ethereumProvider) {
      const blockchainPropertiesClient: BlockchainPropertiesClient =
        yield select(getBlockchainPropertiesClient);
