...
export const RegisterDevice = () => {
  ...
  const externalDeviceIdTypes = useSelector(
    fromGeneralSelectors.getExternalDeviceIdTypes
  );
  ...
  const [externalIdSchema, setExternalIdSchema] = useState({});
  useEffect(() => {
    if (externalDeviceIdTypes) {
      ...
      for (const externalId of requiredDeviceIdTypes) {
        setExternalIdSchema({
          ...externalIdSchema,
          [externalId.type]: Yup.string().required(),
        });
      }
    }
  ...
  async function submitForm(
    ...
    const deviceType = selectedDeviceType.sort(
      (a, b) => b.length - a.length
    )[0];

    formikActions.setSubmitting(true);

    const externalDeviceIds: IExternalDeviceId[] = externalDeviceIdTypes.map(
      ({ type }) => {
        return {
          id: values[type],
          type,
    ...
    dispatch(
      createDevice({
        ...
        commissioningDate: values.commissioningDate.toISOString(),
  ...
  return (
    <Paper className={classes.container}>
      ...
      >
        {(formikProps) => {
          const { isValid, isSubmitting, setFieldValue, validateField } =
            formikProps;
                      ...
                      wrapperProps={{
                        onBlur: (e) => {
                          ...
                          if (!isNaN(parsedValue)) {
                            setFieldValue('latitude', parsedValue.toFixed(2));
                          }
                    ...
                    <FormInput
                      ...
                      wrapperProps={{
                        onBlur: (e) => {
                          ...
                          if (!isNaN(parsedValue)) {
                            setFieldValue('longitude', parsedValue.toFixed(2));
                          }
