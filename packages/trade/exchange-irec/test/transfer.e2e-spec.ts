...
import { authenticatedUser, bootstrapTestInstance, TestUser, deviceManager, ClaimRequestedHandler } from './exchange';
...
describe('Irec exchange transfer controller', () => {
    ...
    before(async () => {
        ({ transferService, databaseService, accountService, app, claimRequestedHandler } = await bootstrapTestInstance());
    ...
    it('should claim irec certificate', async () => {
        ...
        const { body: { id: certificationRequestId } } = await request(app.getHttpServer())
        ...
        const expectedTransferClaimData = {
          ...
          purpose: 'purpose',
          irecVerificationUrl: 'https://baseUrl/public/certificates/en/PwC0IE2x+ugRPJRYhwlGCTjEqk7RDz0x+zzp6Ks0k1M=',
