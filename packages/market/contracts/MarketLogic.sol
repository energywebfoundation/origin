...
contract MarketLogic is Initializable, RoleManagement {
    ...
    function fillDemand(uint _demandId, uint _certificateId) external onlyRole(RoleManagement.Role.Matcher) {
        ...
        // CertificateDefinitions.Certificate memory certificate = TradableEntityDB(originLogicRegistry).getTradableEntity(_certificateId);
        // CertificateLogic(originLogicRegistry).buyCertificateFor(_certificateId, demand.demandOwner);

        // emit DemandPartiallyFilled(_demandId, _certificateId, te.energy);
    }
    ...
    function fillAgreement(uint _demandId, uint _certificateId) external onlyRole(RoleManagement.Role.Matcher) {
        ...
        // TradableEntityContract.TradableEntity memory te = TradableEntityDB(originLogicRegistry).getTradableEntity(_certificateId);
        // CertificateLogic(originLogicRegistry).transferFrom(
        //     te.owner, demand.demandOwner, _certificateId
        // );
