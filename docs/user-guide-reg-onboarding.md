...
You can learn more about MetaMask and how to connect to the Volta Testnetwork [here](https://energy-web-foundation.gitbook.io/energy-web/getting-started-with-ew-dos).    

Once you sign into MetaMask, you will see two Blockchain Addresses:

#### 1. Exchange Deposit Address

![exchangeDepositAddress](images/onboarding/onboarding5-exchangedepositaddress.png)

The Exchange Deposit Address is the address that is used by the marketplace's integrated exchange module. This address is auto-generated by the Origin platform.    

- If you are a member of an organzation that already has an Exchange Deposit Address, this address will be pre-populated after you log connect to MetaMask. 
...
As common practice for blockchain exchanges, this deposit account is tied to the user but _owned by the Exchange operator on-chain_ - this is why the address is auto-generated by the platform and not connected to any account in your MetaMask wallet. By depositing EACs to the Exchange, users put them in the custody of this operator. This ensures that all operations that are performed on the Origin Exchange are valid.

**If you want to post certificates for sale, transfer them to another Exchange Deposit Address, or export them to an I-REC account, they must be in the Exchange account, which is tied to your Organization's Exchange Account Address.** You can choose to withdraw EACs from the Exchange account to your Blockchain account (which is tied to your [Organization's Blockchain Account Address](#organization-blockchain-account-address)) if you want to retire the certificate. *Deposits and withdrawals are the only on-chain events that happen when using the Exchange*.
...
**If you want to withdraw certificates from the exchange in order to retire or claim them for sustainability reporting purposes, they must be in the Blockchain account, which is tied to your Organization's Blockchain Account Address.** You can choose to deposit EACs from the Blockchain account to your Exchange account (which is tied to your [Organization's Exchange Deposit Address](#organization-blockchain-account-address)) if you want to make the certificate eligible for sale on the exchange.  
...
The Blockchain Account Address is the MetaMask address that is used when the admin first sets up the Organization's blockchain account address. **Multiple users from the same organization can have access to this wallet address by importing the private key of this wallet into their MetaMask.** If you or your organization has not connected a blockchain address, select the account that you want to designate as the Blockchain Account Address in MetaMask and click "Connect Blockchain Address." 
